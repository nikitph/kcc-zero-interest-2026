...
KEY=os.environ["ELEVENLABS_API_KEY"]; VOICE=sys.argv[1]; proj=sys.argv[2]; only=sys.argv[3:] 
...
for n,text in lines:
    ...
    body=json.dumps({"text":text,"model_id":"eleven_multilingual_v2","language_code":"hi",
    ...
    mp3=f"{proj}/assets/vo/{n:02d}.mp3"; wav=f"{proj}/assets/vo/{n:02d}.wav"
    ...
    json.dump({"frame":n,"path":f"assets/vo/{n:02d}.wav","duration_s":round(dur,3),"words":wl},open(f"{proj}/assets/vo/{n:02d}.json","w"),ensure_ascii=False,indent=1)
